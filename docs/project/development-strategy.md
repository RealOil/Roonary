...
MVP 1의 감각이 괜찮으면 다음 확장 우선순위를 다시 판단한다.

제품 방향:

```text
Roonary는 최종적으로 단순 집중 타이머보다 루틴/일정 기반 하루 운영 앱을 지향한다.
MVP 1은 현재 루틴을 선택하고 집중 세션처럼 기록하는 작은 루프로 검증한다.
이후에는 오늘의 루틴, 반복 루틴, 일정/캘린더, 위젯, Shared Room 순서로 확장 가능성을 검토한다.
```
